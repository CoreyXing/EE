...
from config import EeArgs
from ee_data_loader import EeDataset, EeCollate
from utils.decode import ner_decode, ner_decode2, bj_decode, sigmoid
from utils.metrics import calculate_metric, classification_report, get_p_r_f
...
class EePipeline:
    ...
    def test(self):
        ...
        with torch.no_grad():
            if "ner" in self.args.tasks:
                metrics = self.eval_forward(test_loader, self.args.entity_label, self.args.ent_id2label,
                                            return_report=True)
                ner_metrics = metrics["ner_metrics"]
        ...
        with torch.no_grad():
            if "ner" in self.args.tasks:
                ...
                attention_masks = torch.from_numpy(np.array(encode_dict['attention_mask'])).unsqueeze(0).to(
                    self.args.device)
                token_type_ids = torch.from_numpy(np.array(encode_dict['token_type_ids'])).unsqueeze(0).to(
                    self.args.device)
                output = self.model(
